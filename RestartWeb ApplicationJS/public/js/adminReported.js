...
async function getResolved()
{
        ...
        for(var st of start)
        {
            if(ctr==0)
            {
                ...
                for(var doc of data.comments)
                {
                    ...
                    "      <div class='col-xl-8 rowDesign'><h5><b>Comment by " + doc.nickName + "</b></h5><p>" +
                     ...
                     "class='btn btn-info'>Remove</button><button onclick='action(\"" + doc.sgid + "\",\""+ doc.postID + "\",\""+ doc.id + "\",\"ignore\")'"+ 
                     "class='btn btn-warning'>ignore</button></div>")
            ...
            "      <div class='col-xl-8 rowDesign'><h5><b>" + doc.title +  " by " + doc.userNickname + "</b></h5><p>" +
             ...
             "class='btn btn-info'>Remove</button><button onclick='actionPost(\"" + doc.sgid + "\",\""+ doc.id + "\",\"ignore\")'"+ 
             "class='btn btn-warning'>ignore</button></div>")
                }
            })
            }
